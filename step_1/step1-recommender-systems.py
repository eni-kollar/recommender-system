...
def svd(user_movie_matrix_normalized):
    pass
...
def predict_latent_factors(movies, users, ratings, predictions):
    ...
    # de-mean the data (normalise each user by their user mean)
    user_movie_matrix_normalized = user_movie_matrix.to_numpy()
    user_ratings_mean = np.mean(user_movie_matrix_normalized, axis=1)
    user_movie_matrix_normalized = user_movie_matrix_normalized - user_ratings_mean.reshape(-1, 1)
    print("user - movie matrix normalised", user_movie_matrix_normalized)
    print(user_movie_matrix_normalized.shape)
    # user_movie_matrix_normalized, user_mean_rating = normalization(user_movie_matrix)
    # svd(user_movie_matrix_normalized)
